Комментарии от turkishjoe
============================
Достался мне проект с этой библиотекой. Убирать ее было не вариант. Неустраивали следующие моменты
1. Конфиг подргужался слующим образом include(dirname(__FILE__) . '/database_config.php');
Это было неудобно. Добавил фикс, теперь его можно подружать с помощью define('DATABASE_CONFIG_FILE', '/path/to/config');    
Фикс быстрый. Возможно позже придумаю что-нибудь поинтереснее.
2. Убрал папку .idea из проекта
3. vendor зачем-то добавлен в проект(пока не проверял, возможно будут проблемы если просто его убрать. Но кажется что его можно убрать). TODO:
4. Возможно имеет смысл перевести данный абзац на английский TODO:
